	...
	nested: (KJar.() -> Unit)? = null)
{
	Jar().execute("jar") { task ->
		...
		if (modificationtime != null)
			task.setModificationtime(modificationtime)
		if (includes != null)
	...
	IServiceNested
{
	...
	fun indexjarsmapper(type: MapperType? = null, classname: String? = null, classpath: String? = null, classpathref: String? = null, from: String? = null, to: String? = null, nested: (KMapper.() -> Unit)? = null) {
		component.addConfiguredIndexJarsMapper(Mapper(component.project).apply {
			component.project.setProjectReference(this)
			_init(type, classname, classpath, classpathref, from, to, nested)
		})
	}
